...
mongoose.connect('mongodb://localhost:27017/clinic',function (err) {
    if (err) {
...
app.get('/allDoc',function(req,res){
    Doctor.find({},function (err, data){
        if (err){
            ...
            res.render("allDoc",{docs: data, moment:moment});
        }
    });
});
...
app.post('/addPtInfo', function(req,res){
    ...
    newPt.save(function(err){
        if (err){
            ...
            res.redirect('/allPt');
            Doctor.updateOne({"_id":mongoose.Types.ObjectId(req.body.doc)},{$inc:{"numPatients":1}}, function (err,doc){
                console.log(doc);
            });
        }
    });
...
app.post("/updateDocInfo", function(req,res){
    Doctor.updateMany({"_id":mongoose.Types.ObjectId(req.body.docID)},{$set:{"numPatients":req.body.newNumber}},function (err,doc){
        res.redirect('/allDoc');
    });
});
